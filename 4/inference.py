...
def constructBayesNet(gameState: hunters.GameState):
    ...
    possibleObservations = [i for i in range(X_RANGE + Y_RANGE - 1 + MAX_NOISE)]
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for key, value in self.items():
            if sample < cumulativeWeight + value:
                return key
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        return 0 if noisyDistance is None else (
            busters.getObservationProbability(noisyDistance, manhattanDistance(pacmanPosition, ghostPosition)))
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        """
        self.particles = []
        legalPositionNumber = len(self.legalPositions)
        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i % legalPositionNumber])
    ...
    def getBeliefDistribution(self):
        ...
        """
        currentBeliefState = DiscreteDistribution()
        for particle in self.particles:
            currentBeliefState[particle] = self.particles.count(particle)
        currentBeliefState.normalize()
        return currentBeliefState
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            weight[particle] += self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
