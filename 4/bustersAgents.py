    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i + 1]]
        mostLikelyLivingGhostPositions = [distribution.argMax() for distribution in livingGhostPositionDistributions]
        closestLivingGhostPosition = min(mostLikelyLivingGhostPositions,
                                         key=lambda position: self.distancer.getDistance(pacmanPosition, position))
        successorPositions = {action: Actions.getSuccessor(pacmanPosition, action) for action in legal}
        return min(legal, key=lambda action: self.distancer.getDistance(closestLivingGhostPosition,
                                                                        successorPositions[action]))
