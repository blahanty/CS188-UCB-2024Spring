...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        return nn.AddBias(nn.Linear(nn.ReLU(
            nn.AddBias(nn.Linear(x, self.w1), self.b1)), self.w2), self.b2)
    ...
    def train(self, dataset):
        ...
        while max_loss > 0.015:
            ...
            for x, y in dataset.iterate_once(batch_size=self.batch_size):
                ...
                max_loss = max(max_loss, nn.as_scalar(loss))
...
class DigitClassificationModel(object):
    ...
    def run(self, x):
        ...
        return nn.AddBias(nn.Linear(nn.ReLU(nn.AddBias(nn.Linear(nn.ReLU(
            nn.AddBias(nn.Linear(x, self.w1), self.b1)), self.w2), self.b2)), self.w3), self.b3)
...
class LanguageIDModel(object):
    ...
    def __init__(self):
        ...
        self.w2 = nn.Parameter(self.hidden_layer_size, self.hidden_layer_size)
        self.b2 = nn.Parameter(1, self.hidden_layer_size)
        self.w3 = nn.Parameter(self.hidden_layer_size, len(self.languages))
    ...
    def run(self, xs):
        ...
        Returns:
            A node with shape (batch_size x 5) containing predicted scores
                (also called logits)
        """
        z = nn.ReLU(nn.AddBias(nn.Linear(xs[0], self.w1), self.b1))
        for x in xs[1:]:
            z = nn.Add(nn.ReLU(nn.AddBias(nn.Linear(x, self.w1), self.b1)),
                       nn.ReLU(nn.AddBias(nn.Linear(z, self.w2), self.b2)))

        return nn.AddBias(nn.Linear(z, self.w3), self.b3)
