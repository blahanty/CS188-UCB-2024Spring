...
def sentence2() -> Expr:
    ...
    (not D) implies C
    """
    A, B, C, D = map(lambda x: Expr(x), 'ABCD')
...
def sentence3() -> Expr:
    ...
    """
    A0, A1 = PropSymbolExpr('PacmanAlive', time=0), PropSymbolExpr('PacmanAlive', time=1)
    B0, K0 = PropSymbolExpr('PacmanBorn', time=0), PropSymbolExpr('PacmanKilled', time=0)
    return conjoin(A1 % ((A0 & ~K0) | (~A0 & B0)), ~(A0 & B0), B0)
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    return conjoin([~e1 | ~e2 for e1, e2 in itertools.combinations(literals, 2)])
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    return findModel(conjoin(KB) & query), findModel(conjoin(KB) & ~query)
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        food_symbol_exprs = []
        for x, y in food:
            food_symbol_expr = PropSymbolExpr(food_str, x, y, time=t)
            KB.append(PropSymbolExpr(food_str, x, y, time=t + 1) % (
                    food_symbol_expr & ~PropSymbolExpr(pacman_str, x, y, time=t)))
            food_symbol_exprs.append(food_symbol_expr)

        model = findModel(conjoin(KB) & ~disjoin(food_symbol_exprs))
        if model:
            return extractActionSequence(model, actions)
...
def localization(problem, agent) -> Generator:
    ...
    KB += [PropSymbolExpr(wall_str, x, y) if (x, y) in walls_list else ~PropSymbolExpr(wall_str, x, y)
           for x, y in all_coords]
